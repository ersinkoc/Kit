...
describe('date utilities', () => {
  ...
  describe('format', () => {
    ...
    it('formats with custom pattern', () => {
      ...
      expect(date.format(d, 'YYYY-MM-DD')).toBe('2024-06-15');
    });

    it('formats with time components', () => {
      const d = new Date(2024, 0, 1, 10, 30, 45, 123);
  ...
  describe('start/end of period', () => {
    it('startOfDay returns midnight', () => {
      const d = new Date(2024, 0, 15, 15, 30, 45);
  ...
  describe('date checks', () => {
    ...
    it('isLeapYear checks leap year', () => {
      expect(date.isLeapYear(2024)).toBe(true);
    ...
    it('isValid checks date validity', () => {
      expect(date.isValid(new Date('2024-01-01'))).toBe(true);
  ...
  describe('calendar info', () => {
    it('daysInMonth returns days in month', () => {
      expect(date.daysInMonth(2024, 2)).toBe(29); // Feb leap year
      expect(date.daysInMonth(2023, 2)).toBe(28); // Feb non-leap
      expect(date.daysInMonth(2024, 1)).toBe(31); // Jan
    });

    it('dayOfYear returns day number', () => {
      expect(date.dayOfYear(new Date(2024, 0, 1))).toBe(1);
      expect(date.dayOfYear(new Date(2024, 1, 1))).toBe(32);
    });

    it('weekOfYear returns week number', () => {
      expect(date.weekOfYear(new Date(2024, 0, 1))).toBeGreaterThanOrEqual(1);
    });

    it('quarterOfYear returns quarter', () => {
      expect(date.quarterOfYear(new Date(2024, 0, 15))).toBe(1);
      expect(date.quarterOfYear(new Date(2024, 3, 15))).toBe(2);
      expect(date.quarterOfYear(new Date(2024, 6, 15))).toBe(3);
      expect(date.quarterOfYear(new Date(2024, 9, 15))).toBe(4);
    });
  });
  ...
  describe('comparison', () => {
    ...
    it('isEqual checks equality', () => {
      const d1 = new Date(2024, 0, 1, 12, 0);
      const d2 = new Date(2024, 0, 1, 12, 0);
      const d3 = new Date(2024, 0, 2, 12, 0);
    ...
    it('clamp clamps date', () => {
      ...
      const inside = new Date(2024, 5, 15);
      const before = new Date(2023, 5, 15);
      const after = new Date(2025, 5, 15);
  ...
  describe('conversions', () => {
    ...
    it('toISOString formats as ISO', () => {
      ...
      expect(date.toISOString(d)).toBe('2024-01-01T12:00:00.000Z');
    });
  ...
  describe('weekdayName/monthName', () => {
    it('weekdayName returns day name', () => {
      const monday = new Date(2024, 0, 1); // Jan 1 2024 was Monday
